...
public class NcpDockerMigrator {
    public void migrateDocker(String[] args, String[] aws){
        ...
        aws[4] = ""; //account id
	*/
	String awsRegion = aws[3].toLowerCase().replace("_","-");
	...
	String awsRegion = aws[3].toLowerCase().replace("_","-");
        ...
        + "& aws configure set aws_secret_access_key "+aws[2]+ "& (aws ecr get-login-password --region "+awsRegion
        + " | docker login --username AWS --password-stdin "+aws[4]+".dkr.ecr."+awsRegion+".amazonaws.com)"};
        ...
        for(int i=0; i<images.length; i++){
            ...
            cmd[2] = "docker push " + aws[4]+".dkr.ecr."+awsRegion+".amazonaws.com/"+aws[0]+":"+images[i];
            System.out.println(cmd[2]);
            p = Runtime.getRuntime().exec(cmd);
            br = new BufferedReader(new InputStreamReader(p.getInputStream()));
            while((s=br.readLine())!=null)
